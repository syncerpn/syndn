...
void free_layer(layer l)
{
    ...
    //nghiant
    if(l.tran_weights)       free(l.tran_weights);
    ...
    if(l.norms_delta)        free(l.norms_delta);
    if(l.skip_index)         free(l.skip_index);
    ...
    //nghiant
    if(l.tran_weights_gpu)        cuda_free(l.tran_weights_gpu);
    if(l.map_gpu)                 cuda_free_int(l.map_gpu);
    if(l.mask)                    cuda_free_int(l.mask_gpu);
    //nghiant_end
    ...
    if(l.norms_delta_gpu)         cuda_free(l.norms_delta_gpu);
    if(l.skip_index_gpu)          cuda_free_int(l.skip_index_gpu);
}