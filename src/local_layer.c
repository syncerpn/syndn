...
void init_local_layer(layer l, initializer init) {
    ...
    initialize_array(l.weights, l.c * l.n * l.size * l.size, init);
...
layer make_local_layer(int batch, int h, int w, int c, int n, int size, int stride, int pad, activation_scheme activation, OPTIMIZER optim)
{
    ...
    if (optim == ADAM) {
        l.m = calloc(c*n*size*size*locations, sizeof(float));
        l.v = calloc(c*n*size*size*locations, sizeof(float));
        l.bias_m = calloc(l.outputs, sizeof(float));
        l.bias_v = calloc(l.outputs, sizeof(float));

        l.m_gpu = cuda_make_array(l.m, c*n*size*size*locations);
        l.v_gpu = cuda_make_array(l.v, c*n*size*size*locations);
        l.bias_m_gpu = cuda_make_array(l.bias_m, l.outputs);
        l.bias_v_gpu = cuda_make_array(l.bias_v, l.outputs);
    }
...
void forward_local_layer_gpu(const layer l, network net)
{
    int out_h = local_out_height(l);
    int out_w = local_out_width(l);
    int i, j;
    ...
    for(i = 0; i < l.batch; ++i){
        ...
        if(net.delta_gpu){
            for(j = 0; j < locations; ++j){ 
                ...
                gemm_gpu(1,0,m,n,k,l.impact,a,m,b,locations,0,c,locations);
            }
...
void update_local_layer_gpu(layer l, update_args a)
{
    ...
    switch (a.optim) {
        ...
        default:
            ...
            scal_gpu(l.outputs, a.momentum, l.bias_updates_gpu, 1);
            ...
            scal_gpu(size, a.momentum, l.weight_updates_gpu, 1);
            break;
    }
}
