...
void init_rnn_layer(layer l, initializer init) {
    init_connected_layer(*(l.input_layer), l.initializer);
    init_connected_layer(*(l.self_layer), l.initializer);
    init_connected_layer(*(l.output_layer), l.initializer);
}
...
layer make_rnn_layer(int batch, int inputs, int outputs, int steps, activation_scheme activation, int batch_normalize, OPTIMIZER optim)
{
    ...
    *(l.input_layer) = make_connected_layer(batch*steps, inputs, outputs, activation, batch_normalize, optim);
    l.input_layer->batch = batch;

    l.self_layer = malloc(sizeof(layer));
    *(l.self_layer) = make_connected_layer(batch*steps, outputs, outputs, activation, batch_normalize, optim);
    l.self_layer->batch = batch;

    l.output_layer = malloc(sizeof(layer));
    *(l.output_layer) = make_connected_layer(batch*steps, outputs, outputs, activation, batch_normalize, optim);
