...
void delta_region_class(float *output, float *delta, int index, int class, int classes, tree *hier, float scale, int stride, int tag)
{
    int i, n;
    if(hier){
        float pred = 1;
    ...
    *(l.cost) = 0;
    for (b = 0; b < l.batch; ++b) {
        ...
        for(t = 0; t < l.max_boxes; ++t){
            ...
            int class_index = entry_index(l, b, best_n*l.w*l.h + j*l.w + i, 4 + 1);
            delta_region_class(l.output, l.delta, class_index, class, l.classes, l.softmax_tree, l.class_scale, l.w*l.h, !l.softmax);
        }
    }
...
void forward_region_layer_gpu(const layer l, network net)
{
    ...
    for (b = 0; b < l.batch; ++b){
        for(n = 0; n < l.n; ++n){
            ...
            index = entry_index(l, b, n*l.w*l.h, 4 + 1);
    ...
    if (l.softmax_tree){
        ...
        int index = entry_index(l, 0, 0, 4 + !l.background);
...
void backward_region_layer_gpu(const layer l, network net)
{
    ...
    for (b = 0; b < l.batch; ++b){
        for(n = 0; n < l.n; ++n){
            int index = entry_index(l, b, n*l.w*l.h, 0);
            gradient_array_gpu(l.output_gpu + index, 2*l.w*l.h, (activation_scheme){LOGISTIC}, l.delta_gpu + index);

            index = entry_index(l, b, n*l.w*l.h, 4);
            if(!l.background) gradient_array_gpu(l.output_gpu + index,   l.w*l.h, (activation_scheme){LOGISTIC}, l.delta_gpu + index);
        }
    }
